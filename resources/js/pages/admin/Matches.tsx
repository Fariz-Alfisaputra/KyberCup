    ...
    return (
        <AdminLayout title="Manajemen Match">
            ...
            <div className="space-y-4">
                <p className="text-sm text-muted-foreground">{matches.total} match terdaftar</p>
                    ...
                    <table className="w-full text-sm">
                        <thead>
                            <tr className="border-b border-border bg-muted/30">
                                ...
                                <th className="px-4 py-3 text-center text-xs font-semibold text-muted-foreground">Skor</th>
                                <th className="px-4 py-3 text-left text-xs font-semibold text-muted-foreground">Round</th>
                        ...
                        <tbody>
                            {matches.data.map((match) => (
                                <tr key={match.id} className="border-b border-border/50 last:border-0 hover:bg-muted/10">
                                    <td className="px-4 py-3 text-muted-foreground">{match.tournament.nama}</td>
                                    <td className="px-4 py-3">
                                        <div className="flex items-center gap-2">
                                            <span className="font-medium">{match.team1?.nama_tim ?? 'TBD'}</span>
                                            <Swords className="h-3.5 w-3.5 text-muted-foreground" />
                                            <span className="font-medium">{match.team2?.nama_tim ?? 'TBD'}</span>
                                        </div>
                                        {match.winner && (
                                    ...
                                    <td className="px-4 py-3 text-center font-bold tabular-nums">
                                        {match.status !== 'scheduled' ? (
                                            <span>{match.skor_team1} - {match.skor_team2}</span>
                                        ) : (
                                            <span className="text-muted-foreground">vs</span>
                                        )}
                                    </td>
                                    <td className="px-4 py-3 text-muted-foreground">Round {match.round}</td>
                                    <td className="px-4 py-3">
                                    ...
                                    <td className="px-4 py-3 text-right">
                                        {match.status !== 'selesai' && match.status !== 'walkover' && (
                                            <button
            ...
            {editingMatch && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm">
                    <div className="w-full max-w-md rounded-2xl border border-border bg-card p-6 shadow-2xl">
                        <h3 className="mb-1 text-lg font-bold">Input Hasil Match</h3>
                        <p className="mb-4 text-sm text-muted-foreground">
                            {editingMatch.team1?.nama_tim} vs {editingMatch.team2?.nama_tim}
                        </p>
                        <form onSubmit={handleSubmit} className="space-y-4">
                            <div>
                                <label className="mb-1.5 block text-sm font-medium">Pemenang</label>
                                <select
                                    value={form.data.winner_id}
                                    onChange={(e) => form.setData('winner_id', e.target.value)}
                                    className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm focus:border-primary focus:outline-none"
                                >
                                    <option value="">Pilih pemenang...</option>
                                    {editingMatch.team1 && (
                                        <option value={editingMatch.team1.id}>{editingMatch.team1.nama_tim}</option>
                                    )}
                                    {editingMatch.team2 && (
                                        <option value={editingMatch.team2.id}>{editingMatch.team2.nama_tim}</option>
                                    )}
                                </select>
                                {form.errors.winner_id && <p className="mt-1 text-xs text-destructive">{form.errors.winner_id}</p>}
                            </div>
                            <div className="grid grid-cols-2 gap-3">
                                <div>
                                    ...
                                    <input
                                        ...
                                        onChange={(e) => form.setData('skor_team1', parseInt(e.target.value))}
                                        className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm focus:border-primary focus:outline-none"
                                    />
                                </div>
                                <div>
                                    ...
                                    <input
                                        ...
                                        onChange={(e) => form.setData('skor_team2', parseInt(e.target.value))}
                                        className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm focus:border-primary focus:outline-none"
                                    />
                                </div>
                            </div>
