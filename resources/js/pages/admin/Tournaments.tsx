...
export default function AdminTournaments({ tournaments }: TournamentsProps) {
    const handleDelete = (id: number, nama: string) => {
        if (
            confirm(
                `Hapus turnamen "${nama}"? Semua data terkait (match, standing, registrasi) akan ikut terhapus.`,
            )
        ) {
    ...
    const handleGenerateBracket = (id: number, nama: string) => {
        if (
            confirm(
                `Generate bracket untuk turnamen "${nama}"? Bracket yang ada akan dihapus dan dibuat ulang.`,
            )
        ) {
            ...
            <div className="space-y-4">
                <div className="flex items-center justify-between">
                    <p className="text-sm text-muted-foreground">
                        {tournaments.total} turnamen terdaftar
                    </p>
                    <Link
