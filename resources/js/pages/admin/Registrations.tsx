...
interface Registration {
    ...
    team: { id: number; nama_tim: string; logo_url: string | null; captain: { name: string } };
}
...
export default function AdminRegistrations({ registrations }: RegistrationsProps) {
        ...
        <AdminLayout title="Manajemen Pendaftaran">
            ...
            <div className="space-y-4">
                <div className="flex items-center justify-between">
                    <p className="text-sm text-muted-foreground">{registrations.total} total pendaftaran</p>
                </div>
                    ...
                    <table className="w-full text-sm">
                        <thead>
                            <tr className="border-b border-border bg-muted/30">
                                ...
                                <th className="px-4 py-3 text-left text-xs font-semibold text-muted-foreground">Captain</th>
                                                ...
                                                {reg.team.logo_url ? (
                                                    ...
                                                    <div className="flex h-7 w-7 items-center justify-center rounded bg-muted">
                                                        <span className="text-xs font-bold text-muted-foreground">
                                                            {reg.team.nama_tim.charAt(0)}
                                                        </span>
                                                    </div>
                                                )}
                                                <span className="font-medium text-foreground">{reg.team.nama_tim}</span>
                                            </div>
                                        </td>
                                        <td className="px-4 py-3">
                                            <p className="font-medium text-foreground">{reg.tournament.nama}</p>
                                            <p className="text-xs text-muted-foreground">{reg.tournament.game.nama_game}</p>
                                        </td>
                                        <td className="px-4 py-3 text-muted-foreground">{reg.team.captain.name}</td>
                                        <td className="px-4 py-3">
